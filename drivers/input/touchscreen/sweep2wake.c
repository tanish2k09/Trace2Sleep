...
#define MIN_SWIPE_DISTANCE  800     // 1080p devices use 800, 720p devices use 400.
#define VIB_STRENGTH 80
...
{
    if ((s2w_scr_suspended) && (s2w_switch !=0))
    {
        if (init == -1)
        {   
            init_x = x;
        ...
        if ( ( (((x - init_x)*(x - init_x)) + ((y - init_y) * (y - init_y))) > ((MIN_SWIPE_DISTANCE) * (MIN_SWIPE_DISTANCE)) ) && (exec_count))
        {
            if ((!device_is_pocketed()) && (!in_phone_call))
            {
